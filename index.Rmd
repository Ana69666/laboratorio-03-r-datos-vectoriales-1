...
names(world)
```
NOTA: recuerde que también puede explorar los datos con funciones como ```str()```, ```print``` y ```View()```.
...
ggtitle("Esperanza de vida en países de Centroamérica")+
xlab("Países") + 
...
plot(centro_america["lifeExp"], main="Esperanza de vida en países de Centroamérica")
...
centro_americamuestra = world[world$name_long == "Belize" & world$lifeExp >= "74"|
...
plot(centro_americamuestra["lifeExp"], 
main="Países de CA con esperanza de vida mayor o igual a 74 años")



kable(
  centro_americamuestra[, c("name_long","lifeExp"), drop = TRUE], 
  col.names=c("País","Esperanza de vida"),  
  caption="Países de CA con esperanza de vida mayor o igual a 74 años"
) %>%
kable_styling()
